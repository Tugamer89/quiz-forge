    ...
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
      <div className="flex items-center justify-between mb-2">
        ...
        >
          ...
          />
          <span className={currentStreak > 0 ? 'text-amber-700 dark:text-amber-500 font-bold' : ''}>
            {currentStreak}
          </span>
        </div>
      </div>
      ...
      <div className="flex w-full mt-4">
        ...
        <div className="flex-1 overflow-hidden flex justify-end p-0.5 -m-0.5" aria-hidden="true">
          <div className="flex flex-col">
            ...
            >
              {Array.from({ length: COLUMNS }).map((_, colIndex) => {
                ...
                return (
                  <div key={label?.name || `col-${colIndex}`} className="relative h-4">
                    {label && (
                      <span
